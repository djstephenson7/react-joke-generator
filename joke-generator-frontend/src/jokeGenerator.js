...
export default class JokeGenerator extends React.Component {
  ...
  loadJoke = () => {
    ...
    // const { data: { value: { joke: { joke } } } } = await axios.get("http://localhost:3001/api/v1/jokes/1");
    // console.log({ data: { value: { joke: { joke } } } })
    //

    axios.get("http://localhost:3001/api/v1/jokes/6")
      .then(result => {
